...
#[tokio::test]
async fn test_url_scraping() -> Result<()> {
    let article = Scraper::try_new("rdbl")?
        .scrape(&Url::parse("https://guides.rubyonrails.org/active_record_basics.html")?)
        .await?;

    assert_eq!(
        article.to_string(),
        r#####"# Active Record Basics

[1. What is Active Record?](#what-is-active-record-questionmark)
----------

Active Record is part of the M in [MVC](https://en.wikipedia.org/wiki/Model%E2%80%93view%E2%80%93controller) - the model - which is the layer of
...
something called an [Object Relational Mapping](https://en.wikipedia.org/wiki/Object-relational_mapping) system. The below sections
explain these terms.

### [1.1. The Active Record Pattern](#the-active-record-pattern) ###

The [Active Record pattern is described by Martin Fowler](https://www.martinfowler.com/eaaCatalog/activeRecord.html) in the book*Patterns of Enterprise Application Architecture* as "an object that wraps a row
...
Record. Please refer to [this SQL tutorial](https://www.khanacademy.org/computing/computer-programming/sql) (or [this RDBMS
tutorial](https://www.devart.com/what-is-rdbms/)) or study them by other means if you would like to learn
more.
...
This uses the [Active Support](https://guides.rubyonrails.org/active_support_core_extensions.html#pluralize)[pluralize](https://api.rubyonrails.org/v8.0.2/classes/ActiveSupport/Inflector.html#method-i-pluralize) method.
...
that case, will be a snake\_case name. For example:
...
* **Primary keys** - By default, Active Record will use an integer column named`id` as the table's primary key (`bigint` for PostgreSQL, MySQL, and MariaDB,`integer` for SQLite). When using [Active Record Migrations](#migrations) to
  create your tables, this column will be automatically created.
* **Foreign keys** - These fields should be named following the pattern`singularized_table_name_id` (e.g., `order_id`, `line_item_id`). These are the
...
* `lock_version` - Adds [optimistic
  locking](https://api.rubyonrails.org/v8.0.2/classes/ActiveRecord/Locking.html) to a
  model.
* `type` - Specifies that the model uses [Single Table
  Inheritance](https://api.rubyonrails.org/v8.0.2/classes/ActiveRecord/Base.html#class-ActiveRecord::Base-label-Single+table+inheritance).
...
* `(table_name)_count` - Used to cache the number of belonging objects on
  associations. For example, if `Article`s have many `Comment`s, a`comments_count` column in the `articles` table will cache the number of
  existing comments for each article.

While these column names are optional, they are reserved by Active Record.
Steer clear of reserved keywords when naming your table's columns. For example,`type` is a reserved keyword used to designate a table using Single Table
...
inherits from[`ActiveRecord::Base`](https://api.rubyonrails.org/v8.0.2/classes/ActiveRecord/Base.html)and it's what turns a regular Ruby class into an Active Record model.
...
That migration creates columns `id`, `title`, `author`, `created_at` and`updated_at`. Each row of this table can be represented by an instance of the`Book` class with the same attributes: `id`, `title`, `author`, `created_at`,
...
author:string`. This creates the files `app/models/book.rb`,`db/migrate/20240220143807_create_books.rb`, and a couple others for testing
purposes.

### [3.1. Creating Namespaced Models](#creating-namespaced-models) ###
...
folder and namespace. For example, `order.rb` and `review.rb` under`app/models/book` with `Book::Order` and `Book::Review` class names,
...
Setting the[table\_name\_prefix](https://api.rubyonrails.org/v8.0.2/classes/ActiveRecord/ModelSchema.html#method-c-table_name_prefix-3D)in `Book` will allow `Order` model's database table to be named`book_orders`, instead of plain `orders`.

The other possibility is that you already have a `Book` model that you want
to keep in `app/models`. In that case, you can choose `n` to not overwrite`book.rb` during the `generate` command.
...
If you do so, you will have to manually define the class name that is hosting[the fixtures](https://guides.rubyonrails.org/testing.html#the-low-down-on-fixtures) (`my_books.yml`) using the`set_fixture_class` method in your test definition:
...
**Active Record does not recommend using non-primary key columns named`id`.** Using a column named `id` which is not a single-column primary key
complicates the access to the column value. The application will have to use the[`id_value`](https://api.rubyonrails.org/v8.0.2/classes/ActiveRecord/ModelSchema.html#method-i-id_value) alias attribute to access the value of the non-PK `id` column.
...
CRUD is an acronym for the four verbs we use to operate on data: **C**reate,**R**ead, **U**pdate, and **D**elete. Active Record automatically creates methods
...
For example, given a `Book` model with attributes of `title` and `author`, the`create` method call will create an object and save a new record to the
database:
